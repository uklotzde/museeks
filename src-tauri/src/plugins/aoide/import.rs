...
use crate::libs::{playlist::Playlist, track::Track};
...
fn number_of(index: Index) -> (Option<u32>, Option<u32>) {
    (index.number.map(Into::into), index.total.map(Into::into))
}
...
pub(crate) fn import_track_from_entity(
    ...
    let (track_no, track_of) = number_of(track.indexes.track);
    let (disk_no, disk_of) = number_of(track.indexes.disc);
    Ok(Track {
        id: uid.to_string(),
        title: track_title,
        ...
        disk_no,
        disk_of,
        path: content_file_path.display().to_string(),
    })
}
